...
resource "aws_route_table" "public" {
  ...
  tags = "${merge(var.tags, var.public_route_table_tags, map("Name", format("%s-dmz-rtb", var.name)))}"
}
...
resource "aws_route_table" "private" {
  ...
  tags = "${merge(var.tags, var.private_route_table_tags, map("Name", (var.single_nat_gateway ? "${var.name}-ecs-rtb" : format("%s-private-%s", var.name, substr(element(var.azs, count.index), -2, 2)))))}"
...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(var.tags, var.public_subnet_tags, map("Name", format("%s-dmz-subnet-%s", var.name, substr(element(var.azs, count.index), -2, 2))))}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(var.tags, var.private_subnet_tags, map("Name", format("%s-ecs-subnet-%s", var.name, substr(element(var.azs, count.index), -2, 2))))}"
}
...
resource "aws_subnet" "database" {
  ...
  tags = "${merge(var.tags, var.database_subnet_tags, map("Name", format("%s-db-%s", var.name, substr(element(var.azs, count.index), -2, 2))))}"
}
